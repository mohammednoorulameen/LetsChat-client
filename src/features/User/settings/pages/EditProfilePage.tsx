...
// 🟢 3. Dummy Data
const editProfileData: EditProfileSection[] = [
  {
    section: "Profile Picture",
    items: [{ label: "Upload New Picture", type: "file", value: "" }],
  },
  {
...
const EditProfilePage: React.FC = () => {
  const [formState, setFormState] = useState<Record<string, string | boolean | File>>(() => {
  ...
  const handleInputChange = (label: string, value: string | boolean | File | null) => {
                  ...
                  >
                    ...
                    {item.type === "toggle" && (
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={Boolean(formState[item.label])}
                    ...
                    {item.type === "input" && (
                      <input
                        type="text"
                        value={formState[item.label] as string}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    ...
                    {item.type === "file" && (
                      <div className="flex flex-col space-y-2 md:w-2/3">
                        ...
                        <input
                          ...
                          accept="image/*"
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            handleInputChange(item.label, e.target.files?.[0] ?? null)
                          }
